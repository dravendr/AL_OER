...
#     grid = GridSearchCV(model,param_grid=param,scoring='neg_mean_absolute_error',cv=10,n_jobs=-1,verbose=-1)
    grid = GridSearchCVProgressBar(model,param_grid=param,scoring='neg_mean_absolute_error',cv=10,n_jobs=-1,verbose=0)
    grid.fit(X,y)
...
fl = open(r'./database_final_full.pkl','rb')
...
model_GPRegressor = GaussianProcessRegressor()
param_gpr = {}
GPR_full,GPR_full_score=gridsearch(model_GPRegressor,param_gpr,'Gaussian Process',data_input_full,data_output_full)
model_SVR = svm.SVR()
param_svr = {
    'coef0': [100], 'degree': [2], 'epsilon': [0.5], 'gamma': ['scale'], 'kernel': ['rbf'], 'max_iter': [1100]
       }
...
param_lgbm = {
'boosting_type': ['gbdt'], 'learning_rate': [0.16], 'max_depth': [7], 'n_estimators': [100], 'reg_alpha': [1e-05], 'reg_lambda': [1e-05], 'subsample': [0.4]
}
...
param_xg={
'booster': ['gbtree'], 'learning_rate': [0.04], 'max_depth': [11], 'n_estimators': [100], 'reg_alpha': [0.01], 'reg_lambda': [0], 'subsample': [0.6]    
}
...
param_cat = {
'boosting_type': ['Plain'], 'learning_rate': [0.05], 'max_depth': [9], 'n_estimators': [400], 'reg_lambda': [0.01], 'subsample': [0.6]
}
...
param_GB = {
    'criterion': ['mae'], 'learning_rate': [0.14], 'loss': ['lad'], 'max_depth': [5], 'max_features': ['sqrt'], 'n_estimators': [400]
}
...
param_dt={
    'criterion': ['mae'], 'max_depth': [6], 'max_features': ['auto'], 'splitter': ['random']
}
...
def create_ANN_model_1layer(X,regular_term=0.001,neuron_number=25,drop_out_rate=0.25,learning_rate=0.001,optimizer='adam'):
    ...
    return model
def create_ANN_model_2layer(X,regular_term=0.001,neuron_number=25,drop_out_rate=0.25,learning_rate=0.001,optimizer='adam'):
    regularizer=keras.regularizers.l2(regular_term)
    model = Sequential() 
    model.add(Dense(neuron_number, input_dim=X.shape[1], kernel_initializer='random_normal',
                    bias_initializer='random_normal',activation='relu',kernel_regularizer=regularizer)) 
    ...
    model.compile(loss='mae', optimizer='adam')
    return model
model_ANNRegressor1= KerasRegressor(build_fn=create_ANN_model_1layer(X=data_input_full), verbose=0)
model_ANNRegressor2= KerasRegressor(build_fn=create_ANN_model_2layer(X=data_input_full), verbose=0)
epochs_list=[]
...
param_ann1={'batch_size': [8], 'epochs': [200], 'optimizer': ['rmsprop']}
param_ann2={'batch_size': [8], 'epochs': [150], 'optimizer': ['adagrad']}
...
def must_need_element_Ir_Ru(x):
    return ((17 in [x[0],x[1],x[2],x[3]]) or (44 in [x[0],x[1],x[2],x[3]]))-1
def sum_prop(x):
...
def predict_func_OVP_GA(p):
    ele_1,ele_2,ele_3,ele_4,prop_1,prop_2,prop_3,prop_4,hydrothermal_temp,hydrothermal_time,hydrothermal_stir,hydrothermal_strong_reduc,hydrothermal_weak_reduc,hydrothermal_solid,annealing_temp,annealing_time,annealing_stir,annealing_inert,annealing_reduc,post_process=p
    ...
    hydrothermal_info=[hydrothermal_temp,hydrothermal_time,hydrothermal_stir]+[hydrothermal_strong_reduc,hydrothermal_weak_reduc,hydrothermal_solid]
    annealing_info=[annealing_temp,annealing_time,annealing_stir,annealing_inert,annealing_reduc,post_process]
    ###合并制备条件
...
def predict_func_variance_GA(p):
    ele_1,ele_2,ele_3,ele_4,prop_1,prop_2,prop_3,prop_4,hydrothermal_temp,hydrothermal_time,hydrothermal_stir,hydrothermal_strong_reduc,hydrothermal_weak_reduc,hydrothermal_solid,annealing_temp,annealing_time,annealing_stir,annealing_inert,annealing_reduc,post_process=p
    test_info=[0.5,0,0,0,5,1]
    ele_info=element_information[element_list[int(ele_1)]]+element_information[element_list[int(ele_2)]]+element_information[element_list[int(ele_3)]]+element_information[element_list[int(ele_4)]]
    prop_info=[prop_1,prop_2,prop_3,prop_4]
...
for i in range(0,10):
    ...
    mode = 'common'
    # mode ='vectorization'
    set_run_mode(predict_func_OVP_GA, mode)
    ga = GA(func=predict_func_OVP_GA, n_dim=20, size_pop=40000, max_iter=50,prob_mut=0.01,lb=[1, 0, 0, 0, 0, 0, 0, 0,25,60,0,0,0,0,200,60,0,0,0,0], ub=[70, 70, 70, 70,100, 100, 100,100,250,1440,1,1,1,1,1000,1440,1,1,1,1],
            constraint_eq=constraint_eq, constraint_ueq=constraint_ueq,precision=[1,1,1,1,1,1,1,1,5,30,1,1,1,1,50,30,1,1,1,1])
