...
fl = open(r'./database_final_full_2nd.pkl','rb')

database_full=pickle.load(fl)
# data_input=database.iloc[:,0:42]
# data_output=database.iloc[:,42]
data_input_full=database_full.iloc[:,0:54]
data_output_full=database_full.iloc[:,54]
model_GPRegressor = GaussianProcessRegressor()
param_gpr = {}
# GPR=gridsearch(model_GPRegressor,param_gpr,'Gaussian Process',data_input,data_output)
...
#              'coef0':[100,200,300,400,500,600,700,800,1000,1100,1200,1300,1400,1500]
    'coef0': [100], 'degree': [2], 'epsilon': [0.3], 'gamma': ['scale'], 'kernel': ['rbf'], 'max_iter': [1000]
       }
...
'boosting_type': ['gbdt'], 'learning_rate': [0.08], 'max_depth': [11], 'n_estimators': [100], 'reg_alpha': [0.01], 'reg_lambda': [0.001], 'subsample': [0.4]
}
...
# 'reg_alpha':[0,0.001,0.01,0.0001,0.00001],
# 'reg_lambda':[0,0.001,0.01,0.0001,0.00001]
'booster': ['gbtree'], 'learning_rate': [0.04], 'max_depth': [9], 'n_estimators': [100], 'reg_alpha': [0.0001], 'reg_lambda': [0.0001], 'subsample': [0.6]
}
...
# 'reg_lambda':[0,0.001,0.01,0.0001,0.00001]
'boosting_type': ['Plain'], 'learning_rate': [0.05], 'max_depth': [7], 'n_estimators': [400], 'reg_lambda': [1e-05,], 'subsample': [0.4]
}
...
'criterion': ['mae'], 'learning_rate': [0.18], 'loss': ['lad'], 'max_depth': [5], 'max_features': ['log2'], 'n_estimators': [100]
}
